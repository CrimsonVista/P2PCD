...
void MyVeinsApp::onBSM(BasicSafetyMessage* bsm)
{
    ...
    bool sameDirection = 1;//(bsm->getSenderDirection() == this->curDirection);
    if (sameDirection) {
        ...
        if (this->curPosition.x != 0) {
            ...
            bool xClosing = (this->curPosition.x > 0 ? (this->curPosition.x > bsm->getSenderPos().x) : (this->curPosition.x < bsm->getSenderPos().x));
        ...
        } else if (this->curPosition.y != 0) {
            EV << "Movement in Y. Check if y will hit and if x is the same" << std::endl;
            bool yClosing = (this->curPosition.y > 0 ? (this->curPosition.y > bsm->getSenderPos().y) : (this->curPosition.y < bsm->getSenderPos().y));
        ...
        if (collisionCourse) {
            ...
            if (bsm->getSenderPos().x > this->curPosition.x) {
